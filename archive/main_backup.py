...
# ── 記憶系統初始化 ─────────────────────────────────────
from rag.memory import init_memory, save_memory, recall_memories
...
async def query_with_memory(question: str) -> str:
    # 1. 搜尋過去記憶
    memory_context = recall_memories(memory_collection, question)  # 同步呼叫，不需要 await
    if memory_context:
    ...
    # 3. 把這次問答存入記憶
    save_memory(memory_collection, question, answer)
